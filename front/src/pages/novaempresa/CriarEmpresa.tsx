...
function DadosNovoEmpresa() {
  const [formData, setFormData] = useState<NovoEmpresaProps>({
    companyID: '', // Identificador único do cliente
    nome: '', // Nome do cliente
    websiteURL: '',
    email: '', // Email do cliente
    telefone: '', // Telefone do cliente
    cidade: '', // Data de nascimento do cliente
    provincia: '', // Gênero do cliente
    data_fundacao: '', // Informação sobre dívidas
    cedula_juridica: '', // Data de registro do cliente
    setor: '', // Código postal
    numero_licenca: '', // Cidade
    rua: '', // Bairro
    bairro: '', // Rua
    canton: '', // Distrito
    distrito: '', // Província
    codigo_postal: '', // Complemento do endereço
    complemento: '', // Canton
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("http://localhost:8000/empresa/novo", {
        method: "POST",
        headers: {
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className="flex flex-col p-10 gap-3">
      {[
        ...
        { label: "Cidade", name: "cidade", type: "date" },
        { label: "Província", name: "provincia", type: "decimal" },
        { label: "Data de Fundação", name: "data_fundacao", type: "text" },
        ...
        { label: "Código Postal", name: "descricao", type: "text" },
